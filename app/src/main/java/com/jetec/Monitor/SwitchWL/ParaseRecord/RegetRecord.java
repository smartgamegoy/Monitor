package com.jetec.Monitor.SwitchWL.ParaseRecord;
...
public class RegetRecord {
    ...
    private String TAG = "RegetRecord";
    ...
    public Map<Integer, String> getRecord(byte[] scanRecord) {
        logMessage.showmessage(TAG, "scanRecord = " + Arrays.toString(scanRecord));
        @SuppressLint("UseSparseArrays")
        Map<Integer, String> ret = new HashMap<>();
        ret.clear();
        int index = 0;
        while (index < scanRecord.length) {
            int length = parase.byteToInt(scanRecord[index++]);
            //Zero value indicates that we are done with the record now
            if (length == 0)
                break;
            int type = parase.byteToInt(scanRecord[index]);
            ...
            if (data.length > 0) {
                ...
                if (type == EBLE_MANDATA) {
                    gethex = gethex.substring(2, 4) +
                            gethex.substring(0, 2) + gethex.substring(4);
                }
                ret.put(type, gethex);
            }
            index += length;
